            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title"><?php echo e(trans('langconvert.menu.login')); ?></h5>
                                ...
                                <form class="card-body border-top-0 pt-4" id="login_form" name="login_form" method="post">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input class="form-control " placeholder="Email"
                                            type="email" value="<?php echo e(old('email')); ?>" name="email" required="" id="username">
...
unset($__errorArgs, $__bag); ?>" placeholder="Enter Captcha" name="captcha">
                                    ...
                                    <div class="text-center mt-3">
                                       <a href="#" id="forgot1" class="text-primary pb-2"><?php echo e(trans('langconvert.menu.forgotpass')); ?></a>
                                        <?php if(setting('REGISTER_DISABLE') == 'on'): ?>
                                        <p class="text-dark mb-0"><?php echo e(trans('langconvert.menu.doyoulogin')); ?><a class="text-primary ms-1" href="#"
                                                data-bs-toggle="modal" id="register1"
                                                data-bs-target="#registermodal"><?php echo e(trans('langconvert.menu.register')); ?></a></p>
                                        <?php endif; ?>
                                    </div>
                                </form>
                            </div>
        ...
        <script type="text/javascript">
            ...
            function login()
            {
                if($('#username').val() == "")
                {
                    $('#err').html('<?php echo e(trans('langconvert.functions.enteremail')); ?>');
                    return false;
                }
                if($('#password').val() == "")
                {
                    $('#passworderr').html('<?php echo e(trans('langconvert.functions.enterpass')); ?>');
                    return false;
                }
                ...
                $.ajax({
                    ...
                    success : function(response)
                    {
                        ...
                        {
                            toastr.error('<?php echo e(trans('langconvert.functions.usernamepassinvalid')); ?>');
                            $("#err").hide().html("<?php echo e(trans('langconvert.functions.usernamepassinvalid')); ?>").fadeIn('slow');
                        }
                        else if(response == 4)
                        {
                            toastr.error('<?php echo e(trans('langconvert.functions.verifyemail')); ?>');
                        }
                        else if(response == 5)
                        {
                            toastr.error('<?php echo e(trans('langconvert.functions.accountdisable')); ?>');
                        }
                    }
                });
            }
        ...
        <!-- Captcha Js -->
        <script src="https://www.google.com/recaptcha/api.js" async defer></script><?php /**PATH /home/bahonbdc/public_html/ticket/resources/views/user/auth/modalspopup/login.blade.php ENDPATH**/ ?>